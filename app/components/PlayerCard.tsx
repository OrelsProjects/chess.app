...
const PlayerCard: React.FC<Parameters> = ({playerName, playerImage, rating, centerText,badge,disabled,onPress,onCancel}) => {
  return (
    <Pressable onPress={()=>onPress?onPress():null} disabled={disabled} activeOpacity={0.5} style={styles.touchable}>
    <View style={styles.playerView} >
      <View style={styles.innerFirstView}>
        <SvgXml xml={playerImage} style={styles.photoStyle} height={normalized.hp(7)} width={normalized.hp(7)} />
        <Text style={styles.nameText}>{playerName}fddf</Text>
      </View>
      {centerText?
      ...
      <View style={styles.innerSecondView}>
        {badge?
        <Text style={styles.badgeText}>WGM</Text>
        :null}
      </View>
